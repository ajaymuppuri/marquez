...
public class JobService {
  ...
  public Job createOrUpdate(
    ...
    try {
      ...
      if (!versionDao.exists(version)) {
        ...
        versionDao.insertAndUpdateWith(newVersionRow, Instant.now());
      }
  ...
  public Run createRun(
    ...
    try {
      ...
      runDao.insertAndUpdateWith(newRunRow, Instant.now());
  ...
  public void markRunAs(@NonNull UUID runId, @NonNull Run.State runState)
      throws MarquezServiceException {
    try {
      final RunStateRow runStateRow = Mapper.toRunStateRow(runId, runState);
      runStateDao.insertAndUpdateWith(runStateRow, Instant.now());
    } catch (UnableToExecuteStatementException e) {
